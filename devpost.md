...
Security tools often treat links as simple strings, while agent workflows treat links as instructions, context, and executable next steps. sancheck was inspired by the need for a real preflight gate that checks a URL before an AI-assisted developer workflow opens, summarizes, or acts on it.

## What it does

sancheck scans URLs and returns an agent-safe verdict. It performs live DNS checks, blocks private and reserved network targets by default, validates HTTPS/TLS, follows redirects safely, checks HTTP health, samples page content, detects prompt-injection patterns, looks up domain age through RDAP, and can call real reputation providers when API keys are configured.
...
sancheck has a real end-to-end scanner, a CLI gate, a local API bridge, and a usable web interface. It does not depend on mocked safety verdicts, and it preserves provider state clearly when optional API keys are not present.
